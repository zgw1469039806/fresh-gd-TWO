...
import org.auth.client.fegin.WxVipLvFeginService;
...
public class WxUserServiceImpl implements GdWxUserService, GdWxUserManageService {
    ...
    @Autowired
    WxVipLvFeginService wxVipLvFeginService;
    ...
    public ResponseData<UserDTO> userinfo(@RequestBody RequestData<String> useraccount) {
        ...
        RequestData<UserAndVipDTO> requestData = new RequestData<>();
        UserAndVipDTO uv = new UserAndVipDTO();
        uv.setUserId(userDTO.getUserId());
        requestData.setData(uv);
        ResponseData<UserAndVipDTO> responseData2 = wxVipFeginService.selectOne(requestData);
        if (responseData2.getData() != null) {
            userDTO.setVipphone(responseData2.getData().getVipphone());
            userDTO.setViplv(responseData2.getData().getViplv());
            RequestData<Integer> requestData2 = new RequestData<>();
            requestData2.setData(responseData2.getData().getViplv());
            ResponseData<GdAddVipLvDTO> responseData3 = wxVipLvFeginService.selVipLvById(requestData2);
            userDTO.setVipdiscount(responseData3.getData().getVipdiscount());
        }
    ...
    @Override
    public ResponseData<GdTakedeliveryDTO> defAddress(@RequestBody RequestData<UserAddressDTO> requestData) {
        ResponseData<GdTakedeliveryDTO> responseData = new ResponseData<>();
        GdTakedelivery d = gdTakedeliveryMapper.queryOne(requestData.getData().getUserid(), requestData.getData()
                .getStatus());
        GdTakedeliveryDTO d2 = new GdTakedeliveryDTO();
        BeanUtils.copyProperties(d, d2);
        responseData.setData(d2);
