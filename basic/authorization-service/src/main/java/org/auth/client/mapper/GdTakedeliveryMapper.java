...
public interface GdTakedeliveryMapper extends BaseMapper<GdTakedelivery> {
    ...
    @Insert("insert into gd_takedelivery(`userid`,`address`,`UPDATED_BY`,`UPDATED_TIME`,`phone`,`consignee`,`status`) values (#{userid},#{address},#{updatedBy},#{updatedTime},#{phone},#{consignee},#{status})")
    ...
    @Select("select * from gd_takedelivery where takedeliveryidid = #{addressId}")
    public GdTakedeliveryDTO selAddressById(Integer addressId);
}
